...
namespace Stock.API.Consumers
{
    public class OrderCreatedEventConsumer : IConsumer<OrderCreatedEvent>
    {
        ...
        public async Task Consume(ConsumeContext<OrderCreatedEvent> context)
        {
            ...
            {
                stockResult.Add(await _context.Stocks.AnyAsync(a => a.ProductID == item.ProductID && a.Count > item.Count));
            }
            if (stockResult.All(x => x.Equals(true)))
            {
                ...
                var sendEndpoint = await _sendEndpointProvider.GetSendEndpoint(new Uri($"queue:{RabbitMQSettingsConst.StockRedervedEventQueueName}"));
